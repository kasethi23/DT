...
class DoomEnvironment():
    ...
    def _gen_actions(self, game, limit_action_space):
        ...
        if buttons == [Button.TURN_LEFT, Button.TURN_RIGHT, Button.MOVE_FORWARD]:
            if limit_action_space:
                feasible_actions = [[True, False, False ], # Left
                                    [False, True, False ],  # Right
                                    [False, False, True],  # Forward
                                    # [True, False, True, ],   # Left + Forward
        ...
        elif self.params.scenario =="basic.cfg":
            # print("reaced here")
            if limit_action_space:
                feasible_actions = [
                ...
                feasible_actions = [
                    ...
                    [False, True, False],  # Right
                    [False, False, True],  # Attack
                    [True, False, True],  # Left + Attack
                    [False, True, True],  # Right + Attack  
                ]         
        else:
            ...
            # print("num of actions",self.params.num_actions)
...
if __name__  == '__main__':
    ...
    # for i in range(1000):
    #     env.game.make_action((1,0,0))

   # Initialize PPO model with CNN policy
    import gym
    import torch
    import numpy as np
    from stable_baselines3 import PPO
    ...
    # Import the custom Doom environment class from your code
    class DoomEnvWrapper(gym.Env):  # Change to inherit from gym.Env
        def __init__(self, env):
            super(DoomEnvWrapper, self).__init__()
            self.env = env  # Store original Doom environment
            self.action_space = gym.spaces.Discrete(env.num_actions)  # Discrete action space
            self.observation_space = gym.spaces.Box(
                low=0, high=255, shape=(3, env.screen_height, env.screen_width), dtype=np.uint8
            )
        ...
        def close(self):
            self.env.game.close()  # Close VizDoom

        @property
        def unwrapped(self):
            return self.env    
    env = DoomEnvWrapper(env)
    vec_env = DummyVecEnv([lambda: env])

    ppo_model = PPO("CnnPolicy", vec_env, verbose=1, device="cuda" if torch.cuda.is_available() else "cpu")
    ...
    # Save trained model
    ppo_model.save("ppo_doom_trained_agent")
    trained_model = PPO.load("ppo_doom_trained_agent")
    ...
    for episode in range(episodes):
        ...
        while not done:
            action, _ = trained_model.predict(obs)  # Predict action using PPO
